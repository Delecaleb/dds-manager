...
foreach ([21644, 22167, 22278] as $pNum) {
    echo "==================== PATIENT $pNum ====================\n";
    ...
    echo "--- All HistAppointments ---\n";
    $hists = DB::table('od_histappointments')->where('PatNum', $pNum)->orderBy('HistDateTStamp')->get();
    foreach ($hists as $h) {
        echo "  Hist {$h->HistApptNum} | Apt {$h->AptNum} | HistDate {$h->HistDateTStamp} | Status {$h->AptStatus} | AptDate {$h->AptDateTime} | Descript: '{$h->ProcDescript}' | Note: {$h->Note}\n";
    }
