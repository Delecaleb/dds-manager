<x-app-layout>
  ...
  <style>
    ...
    .flatpickr-day.inRange {
      background: #d1fae5; border-color: #d1fae5;
    ...
    #bkTable th { background:#f9fafb; font-size:.7rem; font-weight:700; text-transform:uppercase; letter-spacing:.05em; color:#6b7280; border-bottom:1px solid #e5e7eb; padding:.625rem 1rem; white-space:nowrap; }
    #bkTable td { font-size:.8rem; color:#374151; padding:.5rem 1rem; border-bottom:1px solid #f3f4f6; white-space:nowrap; }
    #bkTable tbody tr:hover td { background:#f9fafb; }
    #bkTable tfoot td { font-size:.8rem; font-weight:700; color:#111827; padding:.625rem 1rem; border-top:2px solid #e5e7eb; background:#f9fafb; }
    .bk-patient  { color:#d97706; }
    .bk-provider { color:#0d9488; }
    .bk-prov-id  { color:#0d9488; }
    .bk-count    { color:#3b82f6; }
    .bk-money    { text-align:right; font-variant-numeric:tabular-nums; }
    ...
    .kpi-expand-btn:hover { color:#059669; background:#d1fae5; }
    ...
    #scTable td {
      font-size:.8rem; color:#374151; padding:.5rem 1rem;
      border-bottom:1px solid #f3f4f6; white-space:nowrap;
    }
    #scTable tfoot td {
      font-size:.8rem; font-weight:700; color:#111827;
      padding:.625rem 1rem; background:#f9fafb;
    }
    #scTable tfoot tr:first-child td { border-top:2px solid #e5e7eb; }
    .sc-row-top    { background:#f0fdf4; }
    .sc-row-mid    { background:#fefce8; }
    .sc-row-bottom { background:#fef2f2; }

    /* tier filter buttons */
    .sc-tier-btn {
      padding:.375rem .875rem; border-radius:.375rem; font-size:.75rem; font-weight:600;
      border:1px solid #d1d5db; background:#fff; color:#4b5563; cursor:pointer;
      transition:background .15s, color .15s, border-color .15s;
    }
    .sc-tier-btn:hover { background:#f3f4f6; }
    .sc-tier-btn.active-all    { background:#6b7280; color:#fff; border-color:#6b7280; }
    .sc-tier-btn.active-top    { background:#059669; color:#fff; border-color:#059669; }
    .sc-tier-btn.active-mid    { background:#d97706; color:#fff; border-color:#d97706; }
    .sc-tier-btn.active-bottom { background:#dc2626; color:#fff; border-color:#dc2626; }

    /* main tab active */
    .main-tab { border-bottom:2px solid transparent; padding:.875rem 0 .75rem; font-size:.875rem; font-weight:500; color:#6b7280; cursor:pointer; background:none; border-left:none; border-right:none; border-top:none; }
    .main-tab:hover { color:#374151; }
    .main-tab.active { border-bottom-color:#059669; color:#059669; font-weight:700; }

    /* score sub-tab */
    .sc-sub-tab { padding:.375rem 1.25rem; border-radius:9999px; font-size:.8rem; font-weight:600; cursor:pointer; border:1px solid #d1d5db; background:#fff; color:#4b5563; transition:all .15s; }
    .sc-sub-tab:hover { background:#f3f4f6; }
    .sc-sub-tab.active { background:#059669; color:#fff; border-color:#059669; }
  </style>
  ...
  <main id="summaryPanel" class="p-6 space-y-6 max-w-[1600px] mx-auto">
    ...
    <section class="grid grid-cols-1 md:grid-cols-4 gap-6">
      ...
      <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-xs flex flex-col justify-between">
        ...
        <div class="mt-4 pt-3 border-t border-gray-100 text-xs text-gray-500">
          Target: 80.00% &nbsp;|&nbsp;
          <span id="collections-amt" class="font-semibold text-gray-700"><span class="skel h-3 w-20"></span></span> collected
        </div>
      </div>
    </section>
        ...
        <div class="flex justify-between items-start">
          <div class="flex-1">
            <div class="flex items-center gap-1.5">
              ...
              <button class="kpi-expand-btn" onclick="openBreakdown('avg_production_per_patient','Average Production Per Patient')">
                <i data-lucide="arrow-up-right-square" class="w-4 h-4"></i>
              </button>
            </div>
  ...
  <div id="scoreCardsPanel" class="hidden p-6 space-y-5 max-w-[1600px] mx-auto">
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
      ...
      <div class="flex items-center gap-2">
        <select id="scEntity" class="border border-gray-300 rounded px-3 py-1.5 text-sm focus:outline-none focus:border-emerald-500 text-gray-700">
          <option value="provider">Provider</option>
          <option value="office">Office</option>
        </select>
        <select id="scProvider" class="border border-gray-300 rounded px-3 py-1.5 text-sm focus:outline-none focus:border-emerald-500 text-gray-700" onchange="loadScoreCards()">
    ...
    <div class="bg-white rounded-xl border border-gray-200">
      ...
      {{-- Pagination --}}
      <div class="flex items-center justify-between px-5 py-3 border-t border-gray-200 text-xs text-gray-600 bg-gray-50 rounded-b-xl">
        <div class="flex items-center gap-2">
          <span>Items per page:</span>
          <select id="scPageSize" class="border border-gray-300 rounded px-2 py-0.5 text-xs focus:outline-none focus:border-emerald-400" onchange="scOnPageSize(this.value)">
        ...
        <div class="flex items-center gap-2">
          <select id="scPageNum" class="border border-gray-300 rounded px-2 py-0.5 text-xs focus:outline-none focus:border-emerald-400" onchange="scOnPageSelect(this.value)"></select>
          <span id="scPageOf" class="text-gray-500">of 1 pages</span>
          <button id="scPrev" class="w-7 h-7 flex items-center justify-center border border-gray-300 rounded hover:bg-gray-100 disabled:opacity-40" onclick="scChangePage(-1)">&lsaquo;</button>
          <button id="scNext" class="w-7 h-7 flex items-center justify-center border border-gray-300 rounded hover:bg-gray-100 disabled:opacity-40" onclick="scChangePage(1)">&rsaquo;</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div id="bkOverlay" class="fixed inset-0 z-50 hidden bg-black/60 flex items-center justify-center p-4">
    <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-5xl flex flex-col" style="max-height:90vh">
      ...
      <div class="flex items-center justify-between px-6 py-5 border-b border-gray-200 shrink-0">
        ...
        <button onclick="closeBkModal()" class="text-gray-400 hover:text-gray-600 text-2xl leading-none font-light transition-colors">&times;</button>
      </div>
      ...
      <div class="flex-1 overflow-auto">
        <table id="bkTable" class="w-full border-collapse">
          ...
          <tbody id="bkTbody">
            <tr><td colspan="10" class="text-center py-10 text-gray-400 text-sm">Loading…</td></tr>
          </tbody>
          <tfoot id="bkTfoot"></tfoot>
        </table>
      </div>

      <div class="flex items-center justify-between px-6 py-3 border-t border-gray-200 text-xs text-gray-600 shrink-0 bg-gray-50 rounded-b-xl">
        <div class="flex items-center gap-2">
          <span>Items per page:</span>
          <select id="bkPageSize" class="border border-gray-300 rounded px-2 py-0.5 text-xs focus:outline-none focus:border-emerald-400">
            <option value="10">10</option>
            <option value="20">20</option>
            <option value="50">50</option>
          </select>
        ...
        <div class="flex items-center gap-2">
          <select id="bkPageNum" class="border border-gray-300 rounded px-2 py-0.5 text-xs focus:outline-none focus:border-emerald-400"></select>
          <span id="bkPageOf" class="text-gray-500">of 1 pages</span>
          <button id="bkPrev" class="w-7 h-7 flex items-center justify-center border border-gray-300 rounded hover:bg-gray-100 disabled:opacity-40" onclick="bkChangePage(-1)">&lsaquo;</button>
          <button id="bkNext" class="w-7 h-7 flex items-center justify-center border border-gray-300 rounded hover:bg-gray-100 disabled:opacity-40" onclick="bkChangePage(1)">&rsaquo;</button>
        </div>
      </div>
    </div>
  </div>
  ...
  $('#updateBtn').on('click', function() {
    ...
    if (dates.length === 2) {
      fetchAnalytics(fmtDate(dates[0]), fmtDate(dates[1]));
      if (_sc.data) { _sc.data = null; loadScoreCards(); }
    }
  });
