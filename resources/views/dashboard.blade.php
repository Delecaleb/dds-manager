<x-app-layout>
    <!-- HEADER -->
    <header
        class="h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between shrink-0 sticky top-0 z-10">
        <div class="flex items-center gap-4">
            <!-- Global Multi-Location Context Switcher -->
            <div class="relative">
                <select
                    class="appearance-none bg-slate-50 border border-slate-200 rounded-lg pl-3 pr-10 py-1.5 text-sm font-semibold text-slate-900 focus:outline-none focus:border-blue-500 focus:bg-white cursor-pointer transition-colors">
    ...
    <main class="p-6 space-y-6 max-w-7xl w-full mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                ...
                <div class="text-3xl font-extrabold text-slate-900">${{ number_format($data['production']['gross']) }}
                </div>
            ...
            <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                ...
                <div class="text-3xl font-extrabold text-slate-900">{{ $data['treatment_acceptance']['rate'] ?? 0 }}%
                </div>
            </div>
            <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                <div class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Total Overdue Aging (>90)
                </div>
                <div class="text-3xl font-extrabold text-red-600">${{ number_format($data['aging']['over_90']) ?? 0 }}
                </div>
        ...
        {{-- <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div class="bg-white border border-slate-200 rounded-xl xl:col-span-2 shadow-sm flex flex-col">
                <div class="p-5 border-b border-slate-200 flex items-center justify-between">
                    ...
                    <span class="text-xs font-medium text-blue-600 hover:underline cursor-pointer">Open Full
                        Details</span>
                </div>
                <div class="p-5">
                    <div class="border border-slate-200 rounded-lg overflow-hidden">
                        <div
                            class="grid grid-cols-3 bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-500 p-3">
