...
<div class="space-y-4">
    ...
    {{-- Subtab bar --}}
    <div class="flex items-center gap-1 border-b border-slate-200 overflow-x-auto w-full mb-1">
        @if (!empty($subtabs))
            <ul class="flex px-2 pt-2 gap-1 w-full relative -mb-px">
                @foreach ($subtabs as $slug => $label)
                    <a href="{{ route('operations.tab', $slug === 'default' ? [$tab] : [$tab, $slug]) }}"
                        data-ops-subtab="{{ $slug }}" class="text-[13px] font-bold px-5 py-2.5 rounded-t cursor-pointer whitespace-nowrap transition-colors
                                                        {{ $slug === $subtab
                    ? 'bg-white text-gray-900 border border-b-0 border-slate-200 opacity-100'
    ...
    @if ($subtab !== 'patient-analysis')
        ...
        @if (isset($spec['table_data']))
            ...
            <div class="bg-white border border-slate-200 rounded shadow-sm p-6 mt-6">
                ...
                <div
                    class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 border-b border-gray-100 pb-4">
                    <div>
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
                    ...
                    <div class="flex items-center gap-2 w-full sm:w-auto">
                        <div class="relative w-full sm:w-64">
                            ...
                            <svg class="w-4 h-4 text-slate-400 absolute inset-y-0 right-0 my-auto mr-2.5" fill="none"
                ...
                <div class="overflow-x-auto border border-slate-100 rounded-md max-h-[60vh] relative min-w-full">
                    <table class="w-full text-left border-collapse text-xs whitespace-nowrap" id="marketing_main_table">
                        ...
                        <tbody class="divide-y divide-gray-100 text-gray-700" id="marketing_table_rows_container">
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ count($tableSpec['columns']) }}"
                        ...
                        <tfoot
                            class="sticky bottom-0 z-20 bg-gray-50 border-t border-gray-200 font-bold shadow-[0_-1px_0_0_#e2e8f0]">
                            <!-- Average Row -->
                            @if (!empty($tableSpec['average']))
                                <tr class="bg-gray-50 text-gray-900 border-b border-gray-100">
                                    @foreach ($tableSpec['columns'] as $col)
                                        ...
                                        @endphp
                                        <td
                                            class="py-2.5 px-4 border-r border-gray-200 bg-gray-50/95
                            ...
                            @if (!empty($tableSpec['total']))
                                <tr class="bg-gray-100 text-gray-900 font-bold border-t border-gray-200">
                                    @foreach ($tableSpec['columns'] as $col)
                                        ...
                                        @endphp
                                        <td
                                            class="py-2.5 px-4 border-r border-gray-200 bg-gray-100/95
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mt-6 pt-4 border-t border-gray-100">
                    <div class="flex items-center gap-2 text-xs text-gray-500">
                        ...
                        <select id="marketing_items_per_page"
                            class="border border-gray-300 rounded px-2 py-1 bg-white text-gray-700 focus:outline-none">
                    ...
                    <div class="flex items-center gap-1.5">
                        <button id="marketing_prev_page"
                            class="w-8 h-8 rounded border border-gray-300 flex items-center justify-center bg-white text-gray-500 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7">
                                </path>
                            </svg>
                        </button>
                        <button id="marketing_next_page"
                            class="w-8 h-8 rounded border border-gray-300 flex items-center justify-center bg-white text-gray-500 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
